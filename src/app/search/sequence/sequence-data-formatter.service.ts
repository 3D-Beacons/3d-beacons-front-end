...
export class SequenceDataFormatterService {
  ...
  formatData = (sequenceResponse: any): Array<SequenceResult> => {
    const sequencesResult = sequenceResponse.map((resp: any) => {
      const available_structure = this.getAvailableStructure(
        resp.summary.structures,
      );
      const match_stats = this.getMatchStats(resp.hit_hsps[0]);
      const sequence_stats = this.getSequenceStats(resp.hit_hsps[0]);
      const source_organism = this.getSourceOrganisms(
        resp.hit_uni_os,
        resp.hit_com_os,
      );
      return {
  ...
  getAvailableStructure = (structures: any) => {
    ...
    if (structures) {
      ...
      let uniqueProvider: any = [...new Set(providers)];
  ...
  getIdentities(query: any, match: any) {
  ...
  removeSpacesAndPlusSigns(match: any) {
    let clearmatch = "";
  ...
  getPositives(query: any, match: any) {
    const queryLength = query !== "" ? query.replace(/-/g, "").length : 0;
    let positiveMatch = "";
    for (let i = 0; i < match.length; i++) {
      let character = match[i];
  ...
  getSequenceStats = (hsp: any) => {
    ...
    const queryLength =
      hsp.hsp_qseq !== "" ? hsp.hsp_qseq.replace(/-/g, "").length : 0;
    const alignLength = match.length;
    const targetLength =
      hsp.hsp_hseq !== "" ? hsp.hsp_hseq.replace(/-/g, "").length : 0;
