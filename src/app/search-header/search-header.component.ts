...
export class SearchHeaderComponent implements OnInit {
  ...
  entryData: UniProtEntry | null = null;
  ...
  ngOnInit() {
    this.routeSubscriber = this.router.events.subscribe((val) => {
      if (val instanceof RoutesRecognized) {
        ...
        const paramLength = Object.keys(currentParams ?? {})?.length ?? 0;
        const currentUrl = window.location.pathname;
        if (currentUrl.includes("/sequence") && paramLength === 1) {
          this.el.nativeElement.querySelector(".search-input-field").value =
            localStorage[currentParams?.id]
              ? localStorage[currentParams?.id]
              : "";
          this.el.nativeElement.querySelector(".category-select").value =
            "sequence";
